...
(async () => {
    ...
    let tables = await webService.POST(
    ...
    for (let tableName of tables) {
        ...
        let arrTables = await excell2Database.excel2Array(excelData, tableName)
            .catch(err => {
                console.log(`2. ***>Lỗi Đọc excel tại sheet ${tableName}`, err);
            });
        console.log(`2. Kết quả đọc từ SHEET=${MODEL_NAME} của file excel ${excelData} có số bảng ghi là:`, arrTables.length);
        ...
        let cSLog = await webService.POST(
            ...
            .catch(err => {
                console.log(`3. ***>Lỗi import dữ liệu vào bảng ${MODEL_NAME}/${tableName}`, err);
            });
