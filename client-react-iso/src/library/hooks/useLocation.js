...
export const useLocation = () => {
  const buildState = (trigger) => {
    ...
    return { trigger, state, length, hash, host, hostname, href, origin, pathname, port, protocol, search };
  };
  const [state, setState] = useState(isClient ? buildState("load") : { trigger: "load", length: 1 });
