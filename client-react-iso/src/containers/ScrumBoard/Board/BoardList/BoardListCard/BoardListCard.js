...
import { Avatar, InfoWrapper, Title, CreatedAt, ProjectInfo, MoreActionWrapper } from "./BoardListCard.style";

import { IconSvg } from "@components/ScrumBoard/IconSvg/IconSvg";
import MoreIcon from "@assets/images/icon/16.svg";
import PrivateIcon from "@assets/images/icon/18.svg";
import PublicIcon from "@assets/images/icon/19.svg";
import PlusIcon from "@assets/images/icon/24.svg";
import SoftwareIcon from "@assets/images/icon/20.svg";
...
export default function BoardListCard({ item, onDelete, onEdit }) {
  ...
  return (
    <tbody>
      <tr>
        <td>
          <Link to={`/app/scrum-board/project/${item.id}`}>
            <ProjectInfo>
              <Avatar src={AvatarIcon} alt={item.name} />
              <InfoWrapper>
                <Title>{item.title}</Title>
                <CreatedAt>{moment(item.created_at).format("ddd d, YYYY")}</CreatedAt>
              </InfoWrapper>
            </ProjectInfo>
          </Link>
        </td>
        <td>
          <div style={{ width: 180 }}>
            <Progress percent={parseInt(item.progress)} size="small" status="active" />
          </div>
        </td>
        <td>
