...
const TaskDetials = ({ task, deleteTaskWatcher, editTask, closeDrawer, openDrawer, columnId }) => {
  return (
    <CardDetailsWrapper>
      <CardDetailsHeader
        onBtnClick={() => {
        ...
        <Col span={8}>
          ...
          <p>
            {task.labels.map((label) => (
              <Tooltip title={`${label[0].toUpperCase()}${label.slice(1)}`} placement="bottom" key={label}>
                <Badge status={label} />
              </Tooltip>
            ))}
          </p>
