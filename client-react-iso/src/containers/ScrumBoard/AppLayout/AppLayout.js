...
const AppLayout = ({ children, setSearchText, history, match }) => (
  <Layout style={{ backgroundColor: "#fff", minHeight: "100vh" }}>
    ...
    <HeaderSecondary>
      ...
      <Filters>
        <Popover
          placement="bottom"
          overlayClassName=""
          content={<Checkbox.Group options={ALL_TYPES} defaultValue={["Apple"]} onChange={(value) => console.log(value)} />}
          trigger="click"
        >
          <span>
        ...
        <Popover
          placement="bottom"
          content={<Checkbox.Group options={CATEGORIES} defaultValue={["Apple"]} onChange={(value) => console.log(value)} />}
          trigger="click"
        >
          <span>
            Categories
    ...
    >
      <Scrollbars style={{ width: "100%", height: "calc(100vh - 300px)" }}>{children}</Scrollbars>
    </Content>
  </Layout>
);
