...
import ColumnForm from "@components/scrumBoard/ColumnForm";
import { connect } from "react-redux";
import scrumBoardActions from "@redux/scrumBoard/actions";
import modalActions from "@redux/modal/actions";
import { Container, Header } from "./style";

const Column = ({ title, column, tasks, index, editColumn, cancelEditColumn, deleteColumn, openModal, isScrollable }) => {
  return (
    <Draggable draggableId={title} index={index}>
      {(provided, snapshot) => (
        <Container ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps}>
          {column.editing ? (
            <ColumnForm initials={column} onCancel={() => cancelEditColumn(column)} />
          ) : (
            <Header isDragging={snapshot.isDragging}>
              ...
              />
              <EditOutlined style={{ margin: "0 10px", cursor: "pointer" }} onClick={() => editColumn(column)} />
              <Popconfirm
